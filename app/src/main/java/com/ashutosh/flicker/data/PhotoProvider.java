...
public class PhotoProvider extends ContentProvider {
    ...
    private static UriMatcher buildUriMatcher() {
        ...
        matcher.addURI(authority, "photos", ITEMS);
        matcher.addURI(authority, "photos/#", ITEMS__ID);
        matcher.addURI(authority, Tables.PHOTO_TABLE + "/offset/" + "#", TABLE_ITEMS);
        return matcher;
    }
    ...
    public String getType(Uri uri) {
        ...
        switch (match) {
            case ITEMS:
                return PhotoContract.Photos.CONTENT_TYPE;
            case ITEMS__ID:
                return PhotoContract.Photos.CONTENT_ITEM_TYPE;
            default:
    ...
    public static Uri urlForItems(int limit) {
        return Uri.parse("content://" + PhotoContract.CONTENT_AUTHORITY + "/" + Tables.PHOTO_TABLE + "/offset/" + limit);
    }
    ...
    private SelectionBuilder buildSelection(Uri uri, int match, SelectionBuilder builder) {
        ...
        switch (match) {
            case ITEMS: {
                return builder.table(Tables.PHOTO_TABLE);
            }
            case ITEMS__ID: {
                final String _id = paths.get(1);
                return builder.table(Tables.PHOTO_TABLE).where(PhotoContract.Photos._ID + "=?", _id);
            }
            case TABLE_ITEMS: {
